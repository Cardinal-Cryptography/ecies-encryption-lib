...
async function hkdf(sharedSecret, cryptoAPI) {
...
async function _encrypt(message, recipientPubHex, cryptoAPI) {
    ...
    const ciphertextBuffer = await cryptoAPI.subtle.encrypt({ name: "AES-GCM", iv }, aesKey, message);
...
async function _decrypt(ciphertextBytes, recipientSkHex, cryptoAPI) {
...
export async function encrypt(message, recipientPubHex, cryptoAPI) {
    const encoded = new TextEncoder().encode(message);
    const out = await _encrypt(encoded, recipientPubHex, cryptoAPI);
    return toHex(out);
}
export async function decrypt(ciphertextHex, recipientSkHex, cryptoAPI) {
    const decrypted = await _decrypt(fromHex(ciphertextHex), recipientSkHex, cryptoAPI);
    return new TextDecoder().decode(decrypted);
}
export async function encryptPadded(message, recipientPubHex, cryptoAPI, paddedLength) {
    if (paddedLength < message.length + 4) {
        throw new Error(`Invalid padded length ${paddedLength} bytes, expected at least ${message.length + 4} bytes)`);
    }
...
export async function decryptPadded(ciphertextHex, recipientSkHex, cryptoAPI, paddedLength) {
    const decrypted = await _decrypt(fromHex(ciphertextHex), recipientSkHex, cryptoAPI);
    if (decrypted.length != paddedLength) {
        throw new Error(`Invalid padded length ${decrypted.length} bytes, expected ${paddedLength} bytes)`);
    }
    return decodePadded(decrypted);
}
export async function decryptPaddedUnchecked(ciphertextHex, recipientSkHex, cryptoAPI) {
    const decrypted = await _decrypt(fromHex(ciphertextHex), recipientSkHex, cryptoAPI);
    return await decodePadded(decrypted);
}
async function decodePadded(paddedMessage) {
    if (paddedMessage.length < 4) {
        throw new Error(`Invalid padded length ${paddedMessage.length} bytes, expected at least ${4} bytes)`);
    }
    const view = new DataView(paddedMessage.buffer);
    const messageLength = view.getUint32(0, true);
    if (messageLength > paddedMessage.length - 4) {
        throw new Error(`Invalid message length ${messageLength} bytes, expected at most ${paddedMessage.length - 4} bytes)`);
    }
